...
func solveNQueens1(n int) [][]string {
	...
	for k := range ctx.curBoard {
		ctx.curBoard[k] = make([]string, n)
	}
	for i := 0; i < n; i++ {
		for j := 0; j < n; j++ {
			ctx.curBoard[i][j] = "."
...
func solveNQueens1Inner(ctx *TContext1) {
	if ctx.curRow >= ctx.n {
		...
		for i := 0; i < ctx.n; i++ {
			curRow := strings.Join(ctx.curBoard[i], "")
			oneResult = append(oneResult, curRow)
		}
	...
	for curCol := 0; curCol < ctx.n; curCol++ {
		...
		ctx.curBoard[ctx.curRow][curCol] = "Q"
		...
		ctx.curBoard[ctx.curRow][curCol] = "."
